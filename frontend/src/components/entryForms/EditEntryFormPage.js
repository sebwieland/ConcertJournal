...
const EditEntryFormPage = () => {
    ...
    return (_jsx(EntryForm, { onSubmit: async (formData) => {
        ...
        }, bandName: bandName, setBandName: setBandName, place: place, setPlace: setPlace, date: date, setDate: (newDate) => {
            // Convert the dayjs object to ISO string format (YYYY-MM-DD)
            const isoDate = newDate.toISOString().split("T")[0];
            setDate(isoDate);
        }, rating: rating, setRating: setRating, comment: comment, setComment: setComment, message: message, isSuccess: isSuccess, data: data || [], isUpdate: true, showArtistDetailsButton: false }));
};
export default EditEntryFormPage;
//# sourceMappingURL=EditEntryFormPage.js.map