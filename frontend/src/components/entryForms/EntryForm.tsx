      ...
      const uniqueLocalNames = Array.from(
        new Set(
          data
            ...
            .filter((name) =>
              name.toLowerCase().includes(bandInputValue.toLowerCase()),
            ),
        ),
      );
  ...
  useEffect(() => {
    const fetchSuggestions = () => {
      ...
      const uniquePlaces: string[] = Array.from(
        new Set(data.map((event: ConcertEvent) => event.place)),
      );
      const suggestions: string[] = uniquePlaces.filter((s) =>
        s.toLowerCase().includes(placeInputValue.toLowerCase()),
      );
        ...
        {artistDetails && showArtistDetails && (
          ...
          >
            <Typography variant="body2">
              Type: {artistDetails.type || "Unknown"}
            </Typography>
            <Typography variant="body2">
              Genre: {artistDetails.genre || "Unknown"}
            </Typography>
            <Typography variant="body2">
              Formed: {artistDetails.formationYear || "Unknown"}
            </Typography>
            <Typography variant="body2">
              Country: {artistDetails.country || "Unknown"}
            </Typography>
          </div>
        )}
