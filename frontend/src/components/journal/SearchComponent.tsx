...
const SearchComponent: React.FC<SearchComponentProps> = ({
  ...
  const performSearch = useCallback(
    (searchText: string, sourceData: ConcertEvent[]) => {
              ...
              <SwipeableList style={{ width: "100%" }}>
                {searchResults.map((result) => (
                  <StyledSwipeableListItem
                    key={result.id}
                    leadingActions={
                      onDelete ? leadingActions(onDelete, result.id) : undefined
                    }
                    trailingActions={
                      onEdit ? trailingActions(onEdit, result.id) : undefined
                    }
                  >
                    <Card
                      sx={{
