...
const calculateStatistics = (entries: ConcertEvent[] = []): Statistics => {
  ...
  entries.forEach((item) => {
    ...
    if (!date) {
      if (process.env.NODE_ENV === "development") {
        console.warn("Item missing or invalid date:", item);
      }
      return;
    }
  ...
  const mostSeenArtist = Object.keys(bandCount).reduce((a, b) =>
    bandCount[a] > bandCount[b] ? a : b,
  );
  const mostVisitedLocation = Object.keys(locationCount).reduce((a, b) =>
