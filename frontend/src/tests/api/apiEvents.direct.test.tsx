...
describe("EventsApi", () => {
  ...
  describe("getAllEvents", () => {
    ...
    it("should handle errors when fetching events", async () => {
      ...
      await expect(mockEventsApi.getAllEvents(mockToken)).rejects.toThrow(
        mockError,
      );
    });
  });
  ...
  describe("createEvent", () => {
    ...
    it("should create an event with the correct data and headers", async () => {
      const result = await mockEventsApi.createEvent(
        createEventData,
        mockToken,
      );
    ...
    it("should handle errors when creating an event", async () => {
      ...
      await expect(
        mockEventsApi.createEvent(createEventData, mockToken),
  ...
  describe("updateEvent", () => {
    ...
    it("should update an event with the correct ID, data, and headers", async () => {
      const result = await mockEventsApi.updateEvent(
        1,
        updateEventData,
        mockToken,
      );
    ...
    it("should handle errors when updating an event", async () => {
      ...
      await expect(
        mockEventsApi.updateEvent(1, updateEventData, mockToken),
      ).rejects.toThrow(mockError);
    });
  });
  ...
  describe("deleteEvent", () => {
    ...
    it("should handle errors when deleting an event", async () => {
      ...
      await expect(mockEventsApi.deleteEvent(1, mockToken)).rejects.toThrow(
        mockError,
      );
    });
  });
