...
describe("SignInCard", () => {
    ...
    it("renders the sign-in form with all required elements", () => {
        renderWithProviders(_jsx(SignInCard, { ...mockProps }));
    ...
    it("shows loading state when isLoading is true", () => {
        renderWithProviders(_jsx(SignInCard, { ...mockProps, isLoading: true }));
        expect(screen.getByRole("button", { name: "Loading..." })).toBeInTheDocument();
        expect(screen.getByRole("button")).toBeDisabled();
    });
    it("shows error message for invalid email format", async () => {
        renderWithProviders(_jsx(SignInCard, { ...mockProps, email: "invalid@email" }));
        const emailInput = screen.getByLabelText("Email");
        fireEvent.blur(emailInput);
        expect(screen.getByText("Please enter a valid email address.")).toBeInTheDocument();
    });
    it("shows error message for short password", async () => {
        renderWithProviders(_jsx(SignInCard, { ...mockProps, password: "12345" }));
        const passwordInput = screen.getByLabelText("Password");
...
//# sourceMappingURL=SignInCard.test.js.map