...
describe("EntryForm Component", () => {
    ...
    it("renders the form with all fields", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps }));
        });
    ...
    it("calls the onSubmit function when the form is submitted", async () => {
        ...
        render(_jsx(EntryForm, { ...mockEntryFormProps, onSubmit: onSubmit, bandName: "Test Band", place: "Test Place", date: [2023, 5, 15], rating: 4, comment: "Great show" }));
    ...
    it("displays success message when isSuccess is true", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps, isSuccess: true, message: "Successfully saved!" }));
        });
    ...
    it("displays error message when isSuccess is false and message is provided", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps, isSuccess: false, message: "Error saving data" }));
        });
    ...
    it("updates band name when input changes", async () => {
        ...
        render(_jsx(EntryForm, { ...mockEntryFormProps, setBandName: setBandName }));
    ...
    it("updates place when input changes", async () => {
        ...
        render(_jsx(EntryForm, { ...mockEntryFormProps, setPlace: setPlace }));
    ...
    it("updates comment when input changes", async () => {
        ...
        render(_jsx(EntryForm, { ...mockEntryFormProps, setComment: setComment }));
    ...
    it("updates rating when rating changes", async () => {
        ...
        render(_jsx(EntryForm, { ...mockEntryFormProps, setRating: setRating }));
    ...
    it("shows artist details button when showArtistDetailsButton is true", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps, showArtistDetailsButton: true, bandName: "Test Band" }));
        });
    ...
    it("does not show artist details button when showArtistDetailsButton is false", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps, showArtistDetailsButton: false, bandName: "Test Band" }));
        });
    ...
    it("shows artist details button when bandName is provided", async () => {
        await act(async () => {
            render(_jsx(EntryForm, { ...mockEntryFormProps, bandName: "Test Artist", showArtistDetailsButton: true }));
        });
        // Verify the button is present
        expect(screen.getByRole("button", { name: /artist details/i })).toBeInTheDocument();
    });
});
//# sourceMappingURL=EntryForm.test.js.map